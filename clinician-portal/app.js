...
function renderTimeline(){const items=arr(chart.timeline);return `<div class="clinical-projection"><h2>Longitudinal Timeline</h2>${items.length?items.map(timelineCard).join(''):empty('timeline events')}</div>`;}
