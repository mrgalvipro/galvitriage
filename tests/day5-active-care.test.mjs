...
test('AC-02 clinic brief is read-only same-BHR projection with no OpenAI call',()=>{
  assert.ok(routes.includes('/clinic-brief$/'));
  assert.ok(has(service,['getClinicBrief','brief_fingerprint','read_only:true','gv1_evidence_items','gv1_findings','gv1_hypotheses','gv1_recommendations','gv1_treatment_plans']));
  ...
  assert.equal(brief.includes('INSERT INTO'),false);
  assert.equal(brief.toLowerCase().includes('openai'),false);
});
...
test('AC-03 Business Physician governs finding/Rx/Audit/referral/reassessment',()=>{
  ...
  assert.ok(has(routes,['record_finding_decision','add_galvirx','order_galviaudit','create_referral','reassess_care']));
});

test('AC-04 finding decisions preserve source artifact and append decision version',()=>{
  assert.ok(has(service,['gv1_finding_decisions','source_finding_version','version_no','confirmation_status']));
  assert.equal(service.includes('DELETE FROM gv1_findings'),false);
});
...
test('AC-08 monitoring stays linked to canonical plan/BHR',()=>{
  assert.ok(has(service,['submitCheckin','reportMilestone','reassessCare','planInBmr','Cross-BMR active-care access is prohibited.']));
  assert.ok(has(migration,['idx_checkins_bmr_plan','idx_milestones_bmr_plan','idx_reassessments_bmr_plan']));
});

test('AC-09 readiness requires schema 0006 while preserving signed Day4 cumulative runtime',()=>{
  ...
  assert.ok(entry.includes("required_schema_version:'0006'"));
