...
test('missing source and checksum are quarantinable before mutation',async()=>{
  const result=await validateHistoricalRow({...fixture.rows[0],source_ref:'',source_artifact_checksum:''});
  assert.equal(result.valid,false);assert.deepEqual(result.issues,['source_reference_required','sha256_checksum_required']);
});
...
test('Day 9 routes remain under authenticated operator namespace and public runtime is unchanged',()=>{
  ...
  assert.match(route,/\/api\/v1\/operator\/day9\/historical-import/);assert.match(route,/founder-intelligence-context/);
...
test('synthetic D1 import, exact replay, evidence lineage, FHR projection and intelligence context reconcile without care pollution',async()=>{
  ...
  const context=await composeFounderIntelligenceContext(env,actor,{founderId:first.founder_id,ventureId:first.venture_id,bmrId:first.bmr_id});assert.equal(context.continuity_status,'returning_same_venture');
