...
function addFollowupEvidence(sqlite,s,{stages=['GalviShot'],suffix='followup'}={}){
  const evidence=`evi_${suffix}`;const t=now();
  const followups=stages.map((product,index)=>({product,question_id:`${product.toLowerCase()}_q${index+1}`,question_text:`${product} customer context question`,answer:`Customer-specific ${product} evidence answer`,confidence_impact:8}));
  const content={validation_status:'reported',payload:{legacy_session_id:`gc_${suffix}`,evidence_version:2,followups},provenance:{source:'galvicare_day7d_clinical_followups',captured_server_side:true,customer_session_bound:true}};
  sqlite.prepare("INSERT INTO gv1_evidence_items(evidence_id,bmr_id,session_id,evidence_type,source_product,source_reference,content_json,confidence,evidence_version,created_at) VALUES(?,? ,NULL,'customer_followup','GalviCare',?,?,?,2,?)")
    .run(evidence,s.bmr,`legacy_session:gc_${suffix}`,JSON.stringify(content),.45,t);
  return evidence;
}
function proposal(task,e){
  if(task==='explain_findings')return {findings:[{finding_code:'DISTRIBUTION_BOTTLENECK',statement:'Distribution is the clearest current constraint.',supporting_evidence_ids:[e],contradictory_evidence_ids:[],confidence:.48,reasoning_summary:'The customer-reported follow-up adds context while the low Clinical Confidence keeps this interpretation provisional.',hypothesis_only:true,severity:'moderate',why_it_matters:'Distribution may constrain growth if the reported pattern persists.',next_step:'Collect channel conversion evidence.'}]};
  if(task==='synthesize_evidence')return {summary:'Customer follow-up evidence supports a provisional distribution interpretation while broader causality remains uncertain.',supporting_evidence_ids:[e],contradictory_evidence_ids:[],confidence:.46,implications:['Validate channel performance before changing product strategy.'],hypotheses:[{code:'CHANNEL_EXECUTION_CONSTRAINT',statement:'A channel execution constraint may explain the reported pattern.',supporting_evidence_ids:[e],contradictory_evidence_ids:[],confidence:.42,what_would_change_this:['Evidence of strong acquisition efficiency.']}]};
  if(task==='draft_path')return {objective:'Improve evidence quality around the customer-reported distribution constraint.',sequence:['Collect channel data.','Compare acquisition and retention.','Reassess the provisional finding.'],evidence_required:['Channel conversion evidence.'],cadence:'Reassess after the next evidence cycle.',owner:'customer',escalation:'Escalate to GalviClinic if the constraint remains material.',support_level:'passive_care',supporting_evidence_ids:[e],contradictory_evidence_ids:[],confidence:.44};
  return {hypotheses:[{code:'CHANNEL_EXECUTION_CONSTRAINT',statement:'A channel execution constraint may explain the pattern.',supporting_evidence_ids:[e],contradictory_evidence_ids:[],confidence:.42,what_would_change_this:['New distribution evidence.']}]};
}
...
test('T09 low confidence still requires current-stage evidence and does not call provider',async()=>{const {env,sqlite}=db(),s=seed(sqlite,'low',{confidence:35});let n=0;env.DAY3_PROVIDER_MOCK=async()=>{n++;return{proposal:proposal('explain_findings',s.evidence)}};const x=await call(env,'/api/v1/day3/shot',{suffix:'low',body:{context_id:s.context,current_stage:'GalviShot'}});assert.equal(x.p.status,'needs_evidence');assert.equal(x.p.data.generation_source,'rules');assert.equal(x.p.meta.reasoning_readiness,'needs_current_stage_followup');assert.equal(n,0)});
test('T10 low confidence with answered Shot/Sight/Path follow-up produces bounded governed synthesis without changing Clinical Confidence',async()=>{
  const {env,sqlite}=db(),s=seed(sqlite,'lowanswered',{confidence:35});
  const followup=addFollowupEvidence(sqlite,s,{stages:['GalviShot','GalviSight','GalviPath'],suffix:'lowanswered'});let calls=0;
  env.DAY3_PROVIDER_MOCK=async({task,bundle,deterministic})=>{calls++;assert.equal(deterministic.clinical_confidence,35);assert.equal(bundle.task_constraints.low_clinical_confidence,true);assert.equal(bundle.task_constraints.customer_followup_evidence_supplied,true);assert.ok(bundle.task_constraints.customer_followup_evidence_ids.includes(followup));return{proposal:proposal(task,followup),provider_response_id:`resp_${task}`}};
  const before=sqlite.prepare("SELECT payload_json FROM gv1_day2_intake_results WHERE context_id=? AND result_type='score' ORDER BY record_version DESC LIMIT 1").get(s.context).payload_json;
  for(const [path,stage] of [['shot','GalviShot'],['sight','GalviSight'],['path','GalviPath']]){
    const x=await call(env,`/api/v1/day3/${path}`,{suffix:'lowanswered',body:{context_id:s.context,current_stage:stage}});
    assert.equal(x.r.status,201);assert.equal(x.p.status,'ok');assert.equal(x.p.data.generation_source,'openai_governed');assert.ok(x.p.data.supporting_evidence_ids.includes(followup));assert.equal(x.p.data.reasoning_readiness.sufficient,true);assert.equal(x.p.meta.reasoning_readiness,'followup_evidence_present');
  }
  assert.equal(calls,3);
  const after=sqlite.prepare("SELECT payload_json FROM gv1_day2_intake_results WHERE context_id=? AND result_type='score' ORDER BY record_version DESC LIMIT 1").get(s.context).payload_json;
  assert.equal(after,before);assert.equal(JSON.parse(after).clinical_confidence,35);
});
test('T10B follow-up from a different product cannot bypass the current-stage low-confidence gate',async()=>{const {env,sqlite}=db(),s=seed(sqlite,'wrongstage',{confidence:35});const followup=addFollowupEvidence(sqlite,s,{stages:['GalviShot'],suffix:'wrongstage'});let n=0;env.DAY3_PROVIDER_MOCK=async({task})=>{n++;return{proposal:proposal(task,followup)}};const x=await call(env,'/api/v1/day3/sight',{suffix:'wrongstage',body:{context_id:s.context,current_stage:'GalviSight'}});assert.equal(x.p.status,'needs_evidence');assert.equal(x.p.data.generation_source,'rules');assert.equal(x.p.data.reasoning_readiness.sufficient,false);assert.equal(n,0)});
