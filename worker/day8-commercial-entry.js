...
const COMMERCIAL_REASSESS=/^\/api\/v1\/operator\/commercial-reassessments\/([^/]+)$/;
...
async function commercialForContext(env,contextId){const order=await first(env.DB,'SELECT * FROM gv1_commercial_care_orders WHERE context_id=? ORDER BY created_at DESC LIMIT 1',contextId);if(!order)return{order:null,queue:null};const queue=await first(env.DB,'SELECT queue_id,status,reason_code,source_completion_ref,created_at,customer_confirmed_at,reviewed_at,physician_decision_json FROM gv1_care_reassessment_queue WHERE order_id=?',order.order_id);return{order:{...order,required_completion_keys:parse(order.required_completion_keys_json,[]),completed_completion_keys:parse(order.completed_completion_keys_json,[])},queue:queue?{...queue,physician_decision:parse(queue.physician_decision_json,null)}:null};}
...
export default {async fetch(request,env,executionContext){
  const url=new URL(request.url),path=url.pathname.replace(/\/+$/,'')||'/',ctx=context(request,env),shot=path.match(SHOT),apply=path.match(APPLY),commercialReassess=path.match(COMMERCIAL_REASSESS);
  try{
    ...
    if(shot||apply||commercialReassess||path===REASSESS||path===LIFECYCLE||path===GALVIBOARD){const identity=await requireClinicianIdentity(request,env);if(identity.role!=='business_physician')throw new GVError('GV_AUTH_FORBIDDEN','Business Physician authorization is required.',403);
      ...
      if(commercialReassess&&request.method==='POST'){const data=await recordPreFounderCommercialReassessment(env,ctx,request,identity,decodeURIComponent(commercialReassess[1]),await jsonBody(request));return wrap(success(ctx,data,200,data.idempotent_replay?'no_change':'ok',{idempotent_replay:!!data.idempotent_replay,scenario:'A',canonical_transition:'none',scenario_c:'separate_governed_lifecycle_review_required',manual_repair:'NO'}));}
