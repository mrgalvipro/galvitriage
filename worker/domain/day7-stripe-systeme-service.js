...
export async function createStripeTreatmentOrder(env,ctx,request,key,input={}){
  ...
  const priceEnv=priceEnvFor(serviceCode),priceId=priceEnv?clean(env[priceEnv]):'',courseId=courseIdFor(env,serviceCode),systemeEndpoint=clean(env.SYSTEME_ENROLLMENT_ADAPTER_URL),systemeApiKey=clean(env.SYSTEME_API_KEY);
  ...
  if(!courseId)throw new GVError('GV_SYSTEME_COURSE_NOT_CONFIGURED','The Systeme.io course for this prescribed treatment is not configured. Checkout is blocked so a customer cannot be charged before treatment can be delivered.',503,{required_setting:COURSE_ENV[serviceCode]||null,service_code:serviceCode});
  if(!systemeEndpoint||!systemeApiKey)throw new GVError('GV_SYSTEME_ENROLLMENT_NOT_CONFIGURED','The Systeme.io enrollment channel is not configured. Checkout is blocked so a customer cannot be charged before enrollment is available.',503,{required_settings:['SYSTEME_ENROLLMENT_ADAPTER_URL','SYSTEME_API_KEY'],service_code:serviceCode});
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`INSERT INTO gv1_commercial_order_delivery(order_id,payment_provider,stripe_price_id,stripe_checkout_session_id,stripe_checkout_url,stripe_payment_intent_id,payment_status,payment_verified_at,fulfillment_provider,systeme_course_id,systeme_contact_id,systeme_enrollment_id,enrollment_status,enrollment_attempted_at,enrolled_at,enrollment_error_code,created_at,updated_at) VALUES(?,'stripe',?,?,?,NULL,'checkout_started',NULL,'systeme',?,NULL,NULL,'not_started',NULL,NULL,NULL,?,?)`).bind(orderId,priceId,checkout.id,checkout.url,courseId,ts,ts)
  ]);
  return{order:normalizeOrder(await orderById(env.DB,orderId)),checkout_url:checkout.url,idempotent_replay:false,return_url:successUrl(env)};
}
...
async function requestSystemeEnrollment(env,order,ctx){
  ...
  if(!courseId){await env.DB.prepare("UPDATE gv1_commercial_order_delivery SET enrollment_status='pending_configuration',enrollment_error_code='SYSTEME_COURSE_NOT_CONFIGURED',enrollment_attempted_at=?,updated_at=? WHERE order_id=?").bind(ts,ts,order.order_id).run();return{status:'pending_configuration',reason:'SYSTEME_COURSE_NOT_CONFIGURED'};}
  if(!endpoint||!apiKey){await env.DB.prepare("UPDATE gv1_commercial_order_delivery SET enrollment_status='pending_configuration',enrollment_error_code='SYSTEME_ADAPTER_NOT_CONFIGURED',enrollment_attempted_at=?,systeme_course_id=COALESCE(systeme_course_id,?),updated_at=? WHERE order_id=?").bind(ts,courseId,ts,order.order_id).run();return{status:'pending_configuration',reason:'SYSTEME_ADAPTER_NOT_CONFIGURED'};}
  await env.DB.prepare("UPDATE gv1_commercial_order_delivery SET enrollment_status='requested',enrollment_attempted_at=?,systeme_course_id=COALESCE(systeme_course_id,?),enrollment_error_code=NULL,updated_at=? WHERE order_id=?").bind(ts,courseId,ts,order.order_id).run();
  let response,payload={};try{response=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json','X-API-Key':apiKey},body:JSON.stringify({course_id:courseId,email:order.purchaser_email_normalized,galvicare_order_id:order.order_id,principal_id:order.principal_id,service_code:order.service_code})});payload=await response.json().catch(()=>({}));}catch{await env.DB.prepare("UPDATE gv1_commercial_order_delivery SET enrollment_status='retry_required',enrollment_error_code='SYSTEME_ENROLLMENT_NETWORK',updated_at=? WHERE order_id=?").bind(now(),order.order_id).run();return{status:'retry_required',reason:'SYSTEME_ENROLLMENT_NETWORK'};}
  if(!response.ok){await env.DB.prepare("UPDATE gv1_commercial_order_delivery SET enrollment_status='retry_required',enrollment_error_code='SYSTEME_ENROLLMENT_HTTP',updated_at=? WHERE order_id=?").bind(now(),order.order_id).run();return{status:'retry_required',reason:'SYSTEME_ENROLLMENT_HTTP',http_status:response.status};}
  const enrollmentId=clean(payload.enrollment_id||payload.id),contactId=clean(payload.contact_id||payload.student_id),done=now();
  await env.DB.batch([
    env.DB.prepare("UPDATE gv1_commercial_order_delivery SET enrollment_status='enrolled',systeme_course_id=COALESCE(systeme_course_id,?),systeme_contact_id=COALESCE(?,systeme_contact_id),systeme_enrollment_id=COALESCE(?,systeme_enrollment_id),enrolled_at=COALESCE(enrolled_at,?),enrollment_error_code=NULL,updated_at=? WHERE order_id=?").bind(courseId,contactId||null,enrollmentId||null,done,done,order.order_id),
    env.DB.prepare("UPDATE gv1_commercial_care_orders SET status=CASE WHEN status='paid' THEN 'enrolled' ELSE status END,updated_at=? WHERE order_id=?").bind(done,order.order_id)
  ]);
...
export async function ingestStripeTreatmentWebhook(env,ctx,request){
  ...
  if(type==='checkout.session.completed'||type==='checkout.session.async_payment_succeeded'){
    if(lower(obj.payment_status)!=='paid'){processing='quarantined';failure='GV_STRIPE_PAYMENT_NOT_PAID';}
      ...
      enrollment=await requestSystemeEnrollment(env,await orderById(env.DB,order.order_id),ctx);
    }
