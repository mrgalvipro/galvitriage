...
export async function createStripeTreatmentOrder(env,ctx,request,key,input={}){
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(`INSERT INTO gv1_commercial_order_delivery(order_id,payment_provider,stripe_price_id,stripe_checkout_session_id,stripe_checkout_url,stripe_payment_intent_id,payment_status,payment_verified_at,fulfillment_provider,systeme_course_id,systeme_contact_id,systeme_enrollment_id,enrollment_status,enrollment_attempted_at,enrolled_at,enrollment_error_code,created_at,updated_at) VALUES(?,'stripe',?,?,?,NULL,'checkout_started',NULL,'systeme',?,NULL,NULL,?,NULL,NULL,NULL,?,?)`).bind(orderId,priceId,checkout.id,checkout.url,courseId||null,courseId?'not_started':'pending_configuration',ts,ts)
  ]);
