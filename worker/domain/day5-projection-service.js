...
export async function augmentCustomerChartResponse(response,env){
  ...
  // Day 4 H4.14 established the deployed-D1-safe projection pattern: keep every
  // canonical read simple, sequential, single-table and parameterized, then merge
  // the bounded projection in Worker memory. Do not fan nine D1 statements out
  // through Promise.all; that can pass local SQLite while the deployed D1 path
  // fails the whole Chart read with GV_INTERNAL. This is a read-only projection:
  // no AI regeneration, no canonical write, and no identity/entitlement mutation.
  ...
  const rx=await all(env.DB,`SELECT rx_id,treatment_plan_id,action_id,intervention_code,resource_type,resource_ref,instructions,owner,expected_evidence,cadence,guardrails,status,version_no,created_at,updated_at FROM gv1_galvirx WHERE bmr_id=? AND customer_visible=1 AND status NOT IN ('archived','cancelled') ORDER BY created_at DESC LIMIT 20`,bmrId);
  const audits=await all(env.DB,`SELECT audit_order_id,treatment_plan_id,domain,assigned_service,status,result_evidence_id,created_at,updated_at FROM gv1_galviaudit_orders WHERE bmr_id=? ORDER BY created_at DESC LIMIT 20`,bmrId);
  const referrals=await all(env.DB,`SELECT referral_id,treatment_plan_id,category,provider_name,provider_verification_status,service_mode,geography,consent_status,status,outcome_summary,created_at,updated_at FROM gv1_referrals WHERE bmr_id=? AND consent_status='consented' ORDER BY created_at DESC LIMIT 20`,bmrId);
  const acknowledgements=await all(env.DB,`SELECT treatment_event_id,treatment_plan_id,event_type,occurred_at,created_at FROM gv1_treatment_events WHERE bmr_id=? AND actor_type='customer' AND event_type='customer_acknowledged' ORDER BY occurred_at DESC,created_at DESC LIMIT 20`,bmrId);
  const checkins=await all(env.DB,`SELECT checkin_id,treatment_plan_id,due_context,adherence_state,created_at FROM gv1_checkins WHERE bmr_id=? ORDER BY created_at DESC LIMIT 20`,bmrId);
  const milestones=await all(env.DB,`SELECT milestone_id,treatment_plan_id,milestone_code,status,observed_value,created_at FROM gv1_milestones WHERE bmr_id=? ORDER BY created_at DESC LIMIT 20`,bmrId);
  const outcomes=await all(env.DB,`SELECT outcome_id,treatment_plan_id,outcome_code,outcome_type,outcome_value_json,status,measured_at,created_at FROM gv1_outcomes WHERE bmr_id=? AND status NOT IN ('rejected','archived','superseded') ORDER BY measured_at DESC,created_at DESC LIMIT 20`,bmrId);
  const reassessments=await all(env.DB,`SELECT reassessment_id,treatment_plan_id,decision,created_at FROM gv1_reassessments WHERE bmr_id=? ORDER BY created_at DESC LIMIT 20`,bmrId);
