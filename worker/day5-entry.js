 ...
 * Preserves the complete Day 5 Worker underneath while fixing only the proven
 * Human-E2E provider-size and Score metadata projection defects.
 */
...
async function scoreMetadata(request,env){
  ...
  const legacy=await first(env.DB,`SELECT result_json FROM product_results WHERE session_id=? AND product='GalviScore' LIMIT 1`,sessionId);
  const cp=parseJson(canonical?.payload_json,{}),lp=parseJson(legacy?.result_json,{}),overall=Number(cp?.overall_score??lp?.galviscore_score??lp?.score),dimensions=cp?.dimension_scores||lp?.category_scores||lp?.dimension_scores||{};
  return json(request,{success:true,status:'ok',environment:text(env?.ENVIRONMENT)||'qa',data:{overall_score:Number.isFinite(overall)?overall:null,classification:text(lp?.galviscore_classification||lp?.classification)||classification(overall),lowest_category:text(lp?.galviscore_lowest_category||lp?.lowest_category)||lowestCategory(dimensions),acuity_score:cp?.acuity_score??null,acuity_band:text(cp?.acuity_band)||null,clinical_confidence:cp?.clinical_confidence??lp?.galviscore_confidence??lp?.clinical_confidence??null,score_result_id:canonical?.result_id||null,score_record_version:Number(canonical?.record_version||0),canonical_source:'gv1_day2_intake_results'},meta:{read_only:true,score_recomputed_in_browser:false,acuity_recomputed_in_browser:false,provider_evidence_window_max:MAX_PROVIDER_EVIDENCE_ITEMS}});
}
