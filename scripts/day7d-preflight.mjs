...
// Day 7D is an additive customer-intelligence layer on the proven Day 7A-7C architecture.
// The browser talks to the authoritative API Worker; it must not acquire its own D1 runtime contract.
...
for(const token of ['needs_followup','galvishot-followup-questions','galvisight-followup-questions','galvipath-followup-questions','save_galvishot_followup','save_galvisight_followup','save_galvipath_followup','evidence_version_bumped']) if(!browser.includes(token)) failures.push(`Day 7D browser adapter missing: ${token}`);
...
console.log(JSON.stringify({status:'PASS',environment:'qa',api_worker:wrangler.name,entrypoint:wrangler.main,binding:'DB',database:db.database_name,runtime_schema_adapter:'day7d_dedicated_tables_v1',release_contract:'day7d_progressive_customer_intelligence_v1',progressive_followups:true,evidence_versioned_results:true,legacy_runtime_preservation:true,payment_intelligence_decoupled:true,frontend_d1_required:false},null,2));
