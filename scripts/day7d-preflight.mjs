...
if (wrangler.main !== 'worker/day7d-engine.js') {
  failures.push(`wrangler.json main must be worker/day7d-engine.js, got ${wrangler.main}`);
}
...
for (const token of [
  ...
  'day7d_additive_legacy_preservation_v2',
  'day7d_context_evidence',
  'day7d_observations',
  'legacy_paid_result_preservation',
  'payment_intelligence_decoupled',
...
for (const table of ['day7d_context_evidence','clinical_evidence_versions','day7d_observations']) {
  if (!migration.includes(table)) failures.push(`migration 0006 missing dedicated Day 7D table: ${table}`);
}
...
console.log(JSON.stringify({
  ...
  release_contract: 'day7d_additive_legacy_preservation_v2',
  legacy_paid_result_preservation: true,
