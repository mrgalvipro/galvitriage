...
function replaceAny(oldValues, newText, label) {
  if (html.includes(newText)) return;
  for (const oldText of oldValues) {
    if (html.includes(oldText)) {
      html = html.replace(oldText, newText);
      return;
    }
  }
  throw new Error(`SOURCE DRIFT: ${label}\nNone of the approved source variants were found.`);
}
...
html = html.replace(/<div class="gshot-stage-pill">Current Stage: (?:Chart Your GalviPath|GalviPath Result)<\/div>\s*<h2>Chart Your GalviPath™<\/h2>/g,'<p class="eyebrow">GALVIPATH™ UNLOCKED</p>\n    <h2 class="center">Chart Your GalviPath™</h2>');
html = html.replace(/stateMessage\.textContent\s*=\s*['"](?:Your GalviPath™ is ready|Your deterministic GalviPath is ready)\.['"];/g,"stateMessage.textContent=''; stateMessage.classList.add('hidden');");
...
if(checkupButtonPattern.test(html)&&!html.includes('Book GalviPath™ Check Up to help you Chart Your GalviPath™ with additional clarity &amp; insight.')) {
  html=html.replace(checkupButtonPattern,'<div class="center"><p>Book GalviPath™ Check Up to help you Chart Your GalviPath™ with additional clarity &amp; insight.</p>$1</div>');
}
...
for(const [plain,marked] of brandPairs){
  html=html.replace(new RegExp(`\\b${plain}(?!™)\\b`,'g'),marked);
}
html=html.replace(/™™/g,'™');
...
if(!html.includes('function normalizeCustomerSentence(')){
  const markers=['function scoreClinicalCopy(key)','function vitalStatusLabel(score)'];
  ...
  if(!marker) throw new Error('SOURCE DRIFT: grammar helper insertion marker');
...
const required=[
  ...
  'GALVISIGHT™ UNLOCKED',
  'Your GalviSight Prescription',
  'GALVIPATH™ UNLOCKED',
  'Chart Your GalviPath™',
  'GalviClinician',
